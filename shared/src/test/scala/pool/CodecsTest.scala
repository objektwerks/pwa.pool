...
class CodecsTest extends AnyFunSuite with Matchers {
  ...
  test("pump") {
    ...
    // fails on missing type key - val entityJson = writeToString[Entity](pump)
  ...
  test("heater") {
    ...
    // fails on missing type key - val entityJson = writeToString[Entity](heater)
  ...
  test("timer") {
    ...
    // fails on missing type key - val entityJson = writeToString[Entity](timer)
